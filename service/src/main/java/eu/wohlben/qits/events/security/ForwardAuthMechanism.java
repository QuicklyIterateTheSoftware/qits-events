 ...
 * SecurityIdentity}. It is the whole of this service's relationship with authentication, and it is
 * the same class every qits service carries — duplicated deliberately rather than shared, because
 * each repo builds from a clone of itself alone.
 *
 ...
 * stripping load-bearing rather than tidy. There is no variant to choose and no scheme to configure;
 * a service alone is, by construction, unauthenticated, and its safety is the gateway's and the
 * network's property, never its own.
 *
 ...
 * an oversight: reaching this service at all already implies you are inside the trusted network.
 * {@link #getChallenge} is present because the interface demands it — no policy in this service can
 * reach it.
 */
@ApplicationScoped
